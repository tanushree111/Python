...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for oldPos, oldVal in self.beliefs.items():
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for p in self.legalPositions:
                allPossible[p] += newPosDist[p] * oldVal
    ...
    def observe(self, observation, gameState):
        ...
        if weight.totalCount() == 0:
            ...
            for i in range(len(self.particleList)):
                self.particleList[i] = util.sample(weight)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for j in range(self.numGhosts):
            productList.append(self.legalPositions)
        for element in itertools.product(*productList):
            particleList.append(element)
        for i in range(self.numParticles):
            self.particles.append(particleList[i % len(particleList)])
        ...
        for p in range(len(self.particles)):
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ...
                    trueDistance = util.manhattanDistance(list(self.particles[p])[i], pacmanPosition)
                    prob *= emissionModels[i][trueDistance]
            weight[self.particles[p]] += prob
        weight.normalize()     
        if weight.totalCount() == 0:
            self.initializeParticles()
        else: 
            for i in range(len(self.particles)):
                self.particles[i] = util.sample(weight)
